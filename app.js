...
function cardHtml(entity) {
  ...
  return `
    <button data-open="${entity.id}" class="text-left w-full bg-white hover:bg-amber-50 border border-amber-200 border-l-4 ${c.left} rounded-r-lg rounded-l-sm p-4 transition-colors focus:outline-none focus:ring-2 focus:ring-red-300">
      <div class="flex items-center gap-2 mb-1.5">
        ${icon(iconName, 14, c.text)}
        <h3 class="font-display text-base text-amber-900 truncate">${esc(entity.name || 'Sans nom')}</h3>
      </div>
      ${geo ? `<span class="text-xs font-mono uppercase tracking-wide text-amber-400">${esc(level.label)}</span>` : ''}
      <p class="text-sm text-amber-500 truncate mt-0.5">${esc(summary || 'Aucune description.')}</p>
      ${entity.links.length ? `<div class="flex items-center gap-1 mt-3 text-xs text-amber-400">${icon('link-2', 12)}<span class="font-mono">${entity.links.length} lien${entity.links.length > 1 ? 's' : ''}</span></div>` : ''}
    </button>`;
}
...
function geoCardHtml(node, navigable, color) {
  ...
  return `
    <button data-${navigable ? 'navigate' : 'open'}="${node.id}" class="text-left w-full bg-white hover:bg-amber-50 border border-amber-200 border-l-4 ${color.left} rounded-r-lg rounded-l-sm p-4 transition-colors focus:outline-none focus:ring-2 focus:ring-red-300 flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        ...
        <h3 class="font-display text-base text-amber-900 truncate">${esc(node.name)}</h3>
        <p class="text-sm text-amber-500 truncate mt-0.5">${esc(summary || 'Aucune description.')}</p>
      </div>
      ${navigable ? `<div class="text-amber-400 mt-1 shrink-0">${icon('chevron-right', 16)}</div>` : ''}
    </button>`;
}
...
function geoHeaderHtml(node, color) {
  const level = levelOf(node);
  return `
    <div class="mb-5 bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <span class="text-xs uppercase tracking-wide ${color.text} font-mono">${esc(level.label)}</span>
        <h2 class="font-display text-xl text-amber-900">${esc(node.name)}</h2>
        ${node.fields.description ? `<p class="text-sm text-amber-600 mt-1">${esc(node.fields.description)}</p>` : ''}
      </div>
      <button data-open="${node.id}" class="shrink-0 flex items-center gap-1 text-xs text-amber-500 hover:text-amber-800 bg-white border border-amber-300 hover:border-amber-400 rounded-md px-2 py-1.5 transition-colors">
...
function addChildBarHtml(childTypes, color) {
  ...
  return `<div class="flex flex-wrap gap-2 mb-5">${childTypes.map((ct) => `
    <button data-add-child="${ct}" class="flex items-center gap-1.5 text-xs bg-white border border-amber-300 hover:${color.border} hover:${color.text} text-amber-500 px-3 py-1.5 rounded-md transition-colors">
...
function timelineChainHtml(items, color) {
  return `<div class="flex flex-wrap items-center gap-x-1.5 gap-y-3">${items.map((node, idx) => {
    ...
    return `${idx > 0 ? `<div class="text-amber-300 shrink-0">${icon('arrow-right', 16)}</div>` : ''}
      <button data-${navigable ? 'navigate' : 'open'}="${node.id}" class="flex items-center gap-2 bg-white border border-amber-200 hover:${color.border} rounded-lg px-3 py-2 transition-colors focus:outline-none focus:ring-2 focus:ring-red-300">
        ...
        <span class="flex flex-col items-start min-w-0">
          ...
          <span class="font-display text-sm text-amber-900 truncate max-w-xs">${esc(node.name)}</span>
        </span>
      </button>`;
  }).join('')}</div>`;
}
...
function emptyStateHtml(text) {
  return `<div class="flex flex-col items-center justify-center text-center py-24 text-amber-400">${icon('compass', 28, 'mb-3 text-amber-300')}<p class="text-sm max-w-xs">${esc(text)}</p></div>`;
}
...
function initCategoryPage(categoryKey) {
  ...
  content.innerHTML = `<div class="text-sm text-amber-500 py-10 text-center">Connexion à Firestore…</div>`;
  ...
  function render() {
    ...
    if (isHier) {
      ...
      let breadcrumb = `<div class="flex items-center flex-wrap gap-1 text-xs text-amber-500 mb-4">
        <button data-crumb="-1" class="hover:text-amber-800 ${State.geoPath.length === 0 ? color.text : ''}">${esc(HIERARCHY_ROOT_LABEL[categoryKey])}</button>`;
      State.geoPath.forEach((id, idx) => {
        const node = Store.entities.find((e) => e.id === id);
        if (!node) return;
        breadcrumb += `<div class="text-amber-300">${icon('chevron-right', 12)}</div><button data-crumb="${idx}" class="hover:text-amber-800 ${idx === State.geoPath.length - 1 ? color.text : ''}">${esc(node.name)}</button>`;
      });
      breadcrumb += `</div>`;
  ...
  if (chronoToggle) {
    chronoToggle.querySelectorAll('[data-chrono-view]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        chronoToggle.querySelectorAll('[data-chrono-view]').forEach((b) => b.classList.toggle('bg-amber-200', b === btn));
        render();
      });
    });
  }
...
function fieldHtml(field, value, color) {
  const common = `w-full bg-amber-50 border border-amber-300 rounded-md px-3 py-2 text-sm text-amber-800 placeholder-amber-400 outline-none transition-colors ${color.ring} focus:ring-2 focus:ring-red-200`;
  let control;
  if (field.image) {
    control = value
      ? `<div class="relative inline-block">
           <img src="${value}" class="max-h-40 rounded-md border border-amber-300 object-cover" />
           <button type="button" data-image-clear="${field.key}" class="absolute -top-2 -right-2 bg-white border border-amber-300 rounded-full p-0.5 text-amber-500 hover:text-rose-700">${icon('x', 12)}</button>
         </div>`
      : `<label class="flex items-center justify-center gap-2 border border-dashed border-amber-300 rounded-md py-4 text-xs text-amber-400 cursor-pointer hover:border-amber-400 hover:text-amber-600 transition-colors">
  ...
  return `<div><label class="block text-xs font-mono uppercase tracking-wide text-amber-500 mb-1.5">${esc(field.label)}</label>${control}</div>`;
}

/* ---------------- Relations (Politique) ---------------- */

function relationValueClass(value) {
  if (value > 20) return 'text-emerald-700 bg-emerald-50 border-emerald-200';
  if (value < -20) return 'text-rose-700 bg-rose-50 border-rose-200';
...
function relationsSectionHtml(entity) {
  ...
  const rows = relations.map((r) => {
    ...
    return `<div class="border border-amber-200 rounded-md p-2.5 bg-amber-50 space-y-2" data-relation-row="${r.id}">
      <div class="flex items-center justify-between gap-2">
        <span class="text-sm font-display text-amber-900 truncate">${esc(target ? target.name : '—')}</span>
        <button data-remove-relation="${r.id}" class="text-amber-400 hover:text-rose-700 shrink-0">${icon('x', 13)}</button>
      </div>
      <div class="flex items-center gap-2">
        <select data-relation-type="${r.id}" class="flex-1 bg-white border border-amber-300 rounded-md px-2 py-1 text-xs text-amber-800 outline-none focus:ring-2 focus:ring-red-200">
  ...
  return `<div class="pt-2" data-section="relations">
    <label class="block text-xs font-mono uppercase tracking-wide text-amber-500 mb-2">Relations diplomatiques</label>
    ${relations.length ? `<div class="space-y-2 mb-3">${rows}</div>` : ''}
    <input id="relation-search" placeholder="Lier à une autre faction…" class="w-full bg-amber-50 border border-amber-300 rounded-md px-3 py-2 text-sm placeholder-amber-400 outline-none focus:ring-2 focus:ring-red-200" />
...
function variantsSectionHtml(creature) {
  ...
  return `<div class="pt-2" data-section="variantes">
    <div class="flex items-center justify-between mb-2">
      <label class="text-xs font-mono uppercase tracking-wide text-amber-500">Variantes</label>
      <button id="add-variant-btn" class="flex items-center gap-1 text-xs text-amber-600 hover:text-red-700">${icon('plus', 12)} Ajouter</button>
    </div>
    ${variants.length === 0 ? `<p class="text-xs text-amber-400">Aucune variante. Ajoute un jeune, un alpha, une variante régionale…</p>` :
...
function personalTimelineSectionHtml(entity) {
  ...
  return `<div class="pt-2" data-section="timeline">
    <div class="flex items-center justify-between mb-2">
      <label class="text-xs font-mono uppercase tracking-wide text-amber-500">📜 Chronologie personnelle</label>
      <button id="add-timeline-btn" class="flex items-center gap-1 text-xs text-amber-600 hover:text-red-700">${icon('plus', 12)} Ajouter</button>
    </div>
    ${entries.length === 0 ? `<p class="text-xs text-amber-400">Aucun événement personnel. Ajoute une naissance, une rencontre, un tournant du destin…</p>` :
      `<div class="space-y-2">${entries.map((t) => `
        <div class="border border-amber-200 rounded-md p-2.5 bg-amber-50 space-y-1.5" data-timeline-row="${t.id}">
          <div class="flex items-center gap-2">
            <input value="${esc(t.date)}" data-timeline-date="${t.id}" placeholder="Date" class="w-20 shrink-0 bg-white border border-amber-300 rounded-md px-2 py-1 text-xs font-mono text-amber-800 outline-none focus:ring-2 focus:ring-red-200" />
            <input value="${esc(t.title)}" data-timeline-title="${t.id}" placeholder="Événement" class="flex-1 min-w-0 bg-white border border-amber-300 rounded-md px-2 py-1 text-sm text-amber-900 outline-none focus:ring-2 focus:ring-red-200" />
            <button data-remove-timeline="${t.id}" class="text-amber-400 hover:text-rose-700 shrink-0">${icon('x', 13)}</button>
          </div>
          <textarea rows="2" data-timeline-desc="${t.id}" placeholder="Détails (optionnel)" class="w-full bg-white border border-amber-300 rounded-md px-2 py-1 text-xs text-amber-800 outline-none focus:ring-2 focus:ring-red-200">${esc(t.description)}</textarea>
        </div>`).join('')}</div>`}
  </div>`;
}
...
function renderDrawer() {
  ...
  fields.forEach((f) => {
    if (f.section && f.section !== prevSection) {
      fieldsHtml += `<h4 class="text-xs font-mono uppercase tracking-widest text-amber-400 border-b border-amber-200 pb-1.5 pt-1">${esc(f.section)}</h4>`;
    }
  ...
  const linksHtml = `<div class="pt-2">
    <label class="block text-xs font-mono uppercase tracking-wide text-amber-500 mb-2">Liens</label>
    ${linked.length ? `<div class="flex flex-wrap gap-2 mb-3">${linked.map((le) => {
      ...
      return `<button data-navigate-link="${le.id}" class="flex items-center gap-1 pl-2 pr-1 py-1 rounded-full text-xs border ${lc.bgSoft} ${lc.border} ${lc.text}">
        ...
        <span data-remove-link="${le.id}" class="ml-1 hover:text-amber-900 rounded-full px-1">${icon('x', 10)}</span>
      </button>`;
    }).join('')}</div>` : ''}
    <input id="link-search" placeholder="Rechercher une entité à lier…" class="w-full bg-amber-50 border border-amber-300 rounded-md px-3 py-2 text-sm placeholder-amber-400 outline-none focus:ring-2 focus:ring-red-200" />
  ...
  drawer.innerHTML = `
    <div class="p-5 border-b border-amber-200 bg-amber-50">
      <div class="flex items-start justify-between gap-3">
        <div class="flex-1 min-w-0">
          ...
          <input id="entity-name" value="${esc(entity.name)}" class="w-full bg-transparent font-display text-xl text-amber-900 outline-none border-b border-transparent focus:border-amber-300 pb-1" />
        </div>
        <button id="drawer-close" class="text-amber-400 hover:text-amber-700 p-1">${icon('x', 18)}</button>
      </div>
    </div>
    ...
    <div class="p-4 border-t border-amber-200 flex items-center justify-between">
      <button id="delete-entity" class="flex items-center gap-1.5 text-xs text-rose-700 hover:text-rose-800">${icon('trash-2', 13)} Supprimer cette fiche</button>
      <span class="text-xs text-amber-400">Enregistré automatiquement</span>
    </div>`;
...
function wireDrawerEvents(entity, fields) {
  ...
  linkSearch.addEventListener('input', () => {
    ...
    box.innerHTML = candidates.length ? `<div class="border border-amber-200 rounded-md divide-y divide-amber-200 overflow-hidden">${candidates.map((cand) => {
      const cc = catOf(cand.category);
      const ci = isGeoEntity(cand) ? levelOf(cand).icon : cc.icon;
      return `<button data-pick-link="${cand.id}" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-amber-700 hover:bg-amber-50 text-left">${icon(ci, 13, COLOR_MAP[cc.color].text)}<span class="flex-1 truncate">${esc(cand.name)}</span>${icon('chevron-right', 13, 'text-amber-400')}</button>`;
    }).join('')}</div>` : '';
    refreshIcons();
  ...
  if (entity.category === 'politique') {
    ...
    if (relSearch) relSearch.addEventListener('input', () => {
      ...
      box.innerHTML = candidates.length ? `<div class="border border-amber-200 rounded-md divide-y divide-amber-200 overflow-hidden">${candidates.map((c) => `<button data-pick-relation="${c.id}" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-amber-700 hover:bg-amber-50 text-left"><span class="flex-1 truncate">${esc(c.name)}</span>${icon('chevron-right', 13, 'text-amber-400')}</button>`).join('')}</div>` : '';
      refreshIcons();
